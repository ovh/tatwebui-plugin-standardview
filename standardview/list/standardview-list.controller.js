    ...
    .controller('MessagesStandardViewListCtrl', function (
        ...
        this.mergeMessages = function (dest, source) {
            if (source && _.isArray(source)) {
                for (var i = 0; i < source.length; i++) {
                    var origin = _.find(dest, {_id: source[i]._id});
                    if (origin) {
                        if (!origin.replies) {
                            origin.replies = [];
                        }
        ...
        this.filterSearch = function() {
            ...
            if (self.tmpFilter.idMessage === "-1") {
              $rootScope.$broadcast('topic-change', {topic:self.topic, reload:true});
            } else {
            ...
            if (self.data.isSureWithNoFilter || !self.data.isNoFilter) {
              TatEngineTopicsRsc.list({
                  topic: self.topic
                    ...
                    if (self.data.topic.topic.indexOf("/Private/" + Authentication.getIdentity().username + "/Bookmarks") === 0) {
                        self.data.isTopicBookmarks = true;
                    } else if (self.data.topic.topic.indexOf("/Private/" + Authentication.getIdentity().username + "/Tasks") === 0) {
